...
| **Consequential** | Changed AND the record is outside the veto bands (not settled, not lost) — a disagreement with room to alter what someone does. The dead-deal disagreements are changed but not consequential: whichever verdict prints, the rep does nothing Monday either way. | NOT a competing headline. Changed stays the headline (pure `!=`, zero judgment); consequential lives in the drill-down and Q&A only — never spoken as a second headline number. |
