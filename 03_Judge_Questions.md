...
One read path does exist: **the policy itself lives in the memory layer as a versioned artifact** — v1, provenance-stamped `hand-authored · expert judgment, not learned` — and the app fetches that record from EverOS at load (falling back to the local definition if unreachable; the demo never depends on a live call). So the policy sits alongside the decisions it produced; today a human wrote v1; the loop that writes v2 reads the decisions sitting next to it. **We are explicitly NOT claiming v1 was learned** — the provenance line on screen says so.
